...
public class BruteTest {
    ...
    public void testHeight() {
        ...
        assertArrayEquals(preOrder2, tree.toArrayPreOrder());
    }
    ...
    public void bruteHeightTest(){
        for (int i = 1; i <= 10000; i++){
            ...
            Assert.assertTrue(testBalance((Node<Integer>) tree.getRoot()));
        }
    }
    ...
    public void rightleft(){
        ...
        Assert.assertArrayEquals(new Integer[]{1,  0, -1, 5, 2, 6}, tree.toArrayPreOrder());
    ...
    public void leftRight(){
        ...
        Assert.assertArrayEquals(new Integer[]{5,  3, 2, 4, 7, 8}, tree.toArrayPreOrder());

    }

    private boolean testParents(Node<Integer> node, Node<Integer> parent){

        if (node.isNIL()) return true;

        if (!node.getParent().equals(parent)) return false;

        return testParents((Node<Integer>) node.getLeft(), node) && testParents((Node<Integer>) node.getRight(), node);

    }

    private boolean testBalance(Node<Integer> node){
        if (node.isNIL()) return true;
        int balance = height((Node<Integer>) node.getLeft()) - height((Node<Integer>) node.getRight());
        if (Math.abs(balance) > 1) return false;
        return testBalance((Node<Integer>) node.getLeft()) && testBalance((Node<Integer>) node.getRight());
    }
    ...
    public void fuckingTest(){
        ...
        for (int i = 0; i < 10000; i++){
            ...
            if (k == 0){
                ...
                Assert.assertArrayEquals(myarray.toArray(), tree.toArrayOrder());
                ...
                this.printArray(tree.toArrayOrder());
            ...
            Assert.assertTrue(testParents((Node<Integer>) tree.getRoot(), NIL));
            Assert.assertTrue(testBalance((Node<Integer>) tree.getRoot()));
        }
    ...
    private int height(Node<Integer> node) {
        ...
        return 1 + Math.max(height((Node<Integer>)node.getLeft()), height((Node<Integer>)node.getRight()));
    }
